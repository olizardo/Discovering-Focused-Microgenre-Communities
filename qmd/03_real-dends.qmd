...
```{r Performing Link Clustering}
    ...
    b <- data.frame(g.size = colSums(taste.dat[, 2:21]), 
                    g.names = names(colSums(taste.dat[, 2:21]))) #original macrogenre sizes
...
```{r Creating microgenre dendrograms}
    d3 <- dend.macro$lower$Metal %>% 
      set("labels", c(rep("", nleaves(dend.macro$lower$Metal)))) %>% 
      set("branches_lwd", value = 4) %>% 
      set("branches_k_color", value = brewer.pal(3, "Dark2"), k = 3)
    
    d4 <- dend.macro$lower$Salsa %>% 
      set("labels", c(rep("", nleaves(dend.macro$lower$Salsa)))) %>% 
      set("branches_lwd", value = 4) %>% 
      set("branches_k_color", value = brewer.pal(3, "Dark2"), k = 3)
```
